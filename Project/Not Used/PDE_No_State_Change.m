function f = PDE_No_State_Change(t,L,P,D1,D2)
    Llv=L(:,1); Lsl=L(:,2);
    h=Llv-Lsl; % h is also temporary
    g_prime = -P.C2*P.k2*exp(-P.k2*h) + P.C1*exp(-P.k1*h).*(P.q0*sin(P.q0*h + P.alpha) + P.k2*cos(P.q0*h + P.alpha)) ...
